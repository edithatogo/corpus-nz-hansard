...
SKIP_WORDS = {
    "New Zealand", "Parliament", "Election", "Wikipedia",
    "Minister", "Cabinet", "Speaker", "Whip", "Leader",
    "Electoral", "Aotearoa", "Party", "House", "Representatives",
    "General", "Political", "Electorate", "Monarch", "Governor-General",
    "Sovereign", "Website", "Overview", "File", "Template",
    "Category", "List of", "Main article",
}
...
TEST
        ...
        for tr_match in re.finditer(
            ...
            for a_match in re.finditer(
                ...
                td_match = re.search(
                    r'<td[^>]*>(.*?)</td>', after_name, re.IGNORECASE | re.DOTALL
                )
                if td_match:
                    td_content = td_match.group(1).strip()
                    elec_link = re.search(
                        r'<a[^>]*>([^<]+)</a>', td_content, re.IGNORECASE
                    )
                    if elec_link:
...
def main():
    ...
    output = {
        ...
        "members_by_parliament": {
            str(k): v for k, v in all_mps.items()
        },
    }
    ...
    for p, mps in all_mps.items():
        parties = set(mp["party"] for mp in mps if mp["party"])
