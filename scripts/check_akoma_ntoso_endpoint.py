"""Validate the Akoma Ntoso sample endpoint package and readiness boundary."""
...
MANIFEST_PATH = ROOT / "manifests/akoma_ntoso_validation_manifest.json"
FIXTURE_PATH = ROOT / "fixtures/neutral_components.json"
SAMPLE_XML_PATH = ROOT / "samples/akoma-ntoso/Akoma-Ntoso.sample.xml"
METADATA_XML_PATH = ROOT / "samples/akoma-ntoso/Akoma-Ntoso.metadata.xml"
...
TRACK_PATH = ROOT / "conductor/tracks/akoma_ntoso_endpoint_20260609/evidence.md"
...
def _failures() -> list[str]:
    ...
    unresolved = {
        ...
        if ref not in component_ids and ref != "neutral-fixture-generator"
    }
    if unresolved:
        failures.append(
            f"Akoma Ntoso sample has unresolved neutral references: {sorted(unresolved)}"
        )

    trace_ids = set(manifest["traceability"][0]["neutral_component_ids"])
    if not trace_ids.issubset(component_ids):
        failures.append("Akoma Ntoso traceability cites unknown neutral component IDs.")
    if not trace_ids.issubset(referenced_ids | component_ids):
        failures.append("Akoma Ntoso sample does not expose every traced neutral component ID.")

    if manifest["release_status"] != "sample-not-release":
        failures.append("Akoma Ntoso manifest must remain sample-not-release.")
    if manifest["release_level"] != "upstream-contribution":
        failures.append("Akoma Ntoso sample package must be upstream-contribution level.")
    if set(manifest["dependency_groups"]) != REQUIRED_DEPENDENCY_GROUPS:
        failures.append("Akoma Ntoso dependency groups must match dependency extras policy.")
    if set(manifest["output_artifacts"]) != REQUIRED_OUTPUTS:
        failures.append("Akoma Ntoso validation manifest output artifacts are incomplete.")
    if manifest["validation_results"]["component_metadata_validated"]:
        failures.append("Akoma Ntoso sample must not claim validated component metadata yet.")
    if manifest["validation_results"]["readiness_status"] != "blocked-pending-validated-components":
        failures.append(
            "Akoma Ntoso readiness boundary must remain blocked on validated components."
        )
    if manifest["validation_results"]["blocking_errors"] != 0:
        failures.append("Akoma Ntoso sample integrity check must have zero blocking errors.")
    ...
    for relative_path, text in {
        "docs/akoma-ntoso-mapping.md": _read(MAPPING_DOC_PATH),
        "docs/endpoint-contracts.md": _read(ENDPOINT_DOC_PATH),
        "samples/akoma-ntoso/README.md": _read(SAMPLE_README_PATH),
    }.items():
        for term in required_terms:
            if term not in text:
                failures.append(f"{relative_path} is missing Akoma Ntoso term: {term}")
...
def main() -> int:
    ...
    if failures:
        for failure in failures:
            print(f"AKOMA-NTOSO: {failure}")
        return 1
    print("Akoma Ntoso sample endpoint is consistent.")
    return 0
