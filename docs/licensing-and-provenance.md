# Licensing and Provenance

## Source Material

The source artifact in this workspace is:

- `2024-09-06 Hansard Extract from DocumentsDB.zip`

It contains New Zealand Parliament Hansard CSV extracts for Parliament numbers 47 through 54.

## Official Hansard Status

New Zealand Parliament's Parliamentary Practice chapter states that:

- Hansard is the official report of debates in the House.
- Hansard is published on the Parliament website and as New Zealand Parliamentary Debates.
- Digitised copies of NZPD volumes are available through the Parliament website.
- No copyright exists in Hansard / New Zealand Parliamentary Debates.

Official source:

- `https://www3.parliament.nz/en/visit-and-learn/how-parliament-works/parliamentary-practice-in-new-zealand-2023-by-chapter/chapter-12-communicating-and-engaging-with-parliament/`

Relevant sections:

- 12.3 Hansard
- 12.10 Copyright, licences, and terms of use

## Reuse Caveats

- This project should not imply New Zealand Parliament endorses the dataset or downstream analyses.
- Official coverage, video, audio, website material, and third-party material may have separate rules or conditions.
- This dataset is built from text records in the supplied DocumentsDB extract and should not be described as an official publication channel.
- Public dataset publication should include a clear attribution and provenance statement.

## Suggested Attribution

Contains New Zealand Parliamentary Debates (Hansard) material sourced from a DocumentsDB extract dated 2024-09-06. Hansard is the official report of debates in the New Zealand House of Representatives. This derived dataset is not endorsed by New Zealand Parliament.

## Publication Status

Status: prepared for review, not published.

Publication still needs:

- Final human licensing/provenance review.
- Hosting decision.
- Dataset version identifier.
- Release artifact policy.
- Public-facing limitations statement.
